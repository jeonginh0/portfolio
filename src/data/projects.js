...
export const projects = [
    {
        ...
        detail: {
            ...
            period: '2025.02.01 ~ 2025.10.15 (8개월, 서비스 중)',
    ...
    },
    {
        ...
        detail: {
            summary:
                ...
                'JWT Access/Refresh 인증을 구현했고, 부품 조회 API는 PostgreSQL GIN 인덱스(pg_trgm) 적용으로 응답속도를 92% 이상 개선했습니다.',
            role: '프로젝트 기획 및 백엔드 개발 — 시스템 설계, API 구현, 인증 시스템 구현, 성능 최적화',
            period: '2025.09.09 ~ 2025.12.12 (3개월)',
            ...
            contributions: [
                '유저, 채팅방, 대화내역, 부품, 견적 엔티티 설계 및 구현',
                '시스템 흐름 구성 및 API 설계 (회원가입/인증, 부품 조회, 견적 생성)',
                'JWT Access/Refresh 인증 기능 구현 (Redis 기반 Refresh Token 관리 및 Access Token 블랙리스트)',
                'PostgreSQL GIN 인덱스(pg_trgm) 적용으로 부품 조회 API 성능 개선',
            ],
            trouble: [
                '부품 조회 API는 카테고리, 제조사, 키워드, 정렬, 페이지 번호/크기 등 6가지 파라미터 조합에 따라 ' +
                '매 요청마다 DB 전체 스캔 쿼리가 실행되었습니다. ' +
                '특히 키워드 검색의 LIKE \'%keyword%\' 조건(중간 포함 검색)은 일반 B-tree 인덱스로는 전방 일치만 타서 ' +
                '풀스캔으로 처리되어 응답 지연이 심각했습니다. ' +
                '처음엔 Redis 캐싱으로 우회했지만 가격·재고처럼 자주 바뀌는 데이터라 캐시 무효화 이슈가 발생해, ' +
                'PostgreSQL의 pg_trgm 확장을 활용한 GIN 인덱스와 속성 필터링·정렬용 복합 B-tree 인덱스를 구성하는 ' +
                'DB 자체 조회 성능 개선 방향으로 전환했습니다. ' +
                'EXPLAIN ANALYZE로 플래너 동작을 검증해 데이터가 증가할수록 GIN 인덱스를 자동 선택하도록 설계했으며, ' +
                '결과적으로 조회 API 실행 시간을 166ms에서 6ms로 약 92% 단축했습니다.',
            ],
        },
    ...
    },
    {
        ...
        detail: {
            ...
            period: '2025.03.09 ~ 2025.06.12 (3개월)',
